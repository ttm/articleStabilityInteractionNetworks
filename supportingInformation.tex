...
and the relation between the minimum and the maximum incidences $\frac{min(incidence)}{max(incidence)}$ at each time scale.
Also, the mean $ \mu_{\frac{min(incidence')}{max(incidence')}} $ and 
the standard deviation $ \sigma_{\frac{min(incidence')}{max(incidence')} }$ 
of the relation between the minimum and the maximum incidences
...
\begin{center}
    \begin{tabular}{ |l|| c|c|c|c|c||c|c| }
        \hline
scale & $\theta_\mu'$ & $S(z)$ & $Var(z)$ & $\delta(z)$ & $\frac{min(incidence)}{max(incidence)}$ & $ \mu_{\frac{min(incidence')}{max(incidence')}} $ & $ \sigma_{\frac{min(incidence')}{max(incidence')} } $ \\ \hline\hline
