...
\begin{table*}[t]
	\caption{Circular mean, standard deviation, variance, circular dispersion and the relation of maximum and minimum incidence at each time unit. The Appendix~\ref{ap:circular} exposes the theoretical background considered of directional (or circular) statistics.}
\begin{center}
    \begin{tabular}{ |l|| c|c|c|c|c| }
        \hline
	scale & mean & standard deviation & variance & circular dispersion & $\frac{max(incidence)}{min(incidence)}$ \\ \hline\hline
...
\begin{figure*} 
   \centering
        \includegraphics[width=\textwidth,height=12cm]{figs/im13PCAPLOT}
...
\begin{itemize}
	...
	\item It is given by construction that a participant is of just one type at a network in a given snapshot (exception for inclusive compound criteria given in Section~\ref{sectioning}).
