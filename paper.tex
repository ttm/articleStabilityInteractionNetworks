...
Research on network evolution often considers solely network growth, in which there is a monotonic increase in the number of events~\cite{barabasiEvo}. Exceptions are reported in this section, with emphasis on those more closely related to the present article.
...
\subsection{Data and scripts availability}\label{scripts}
As a way to share routines with the scientific community and the whole of the society,
all the software necessary to achieve the results reported in this article,
including all tables and figure of the Supporting Information,
are available through a public domain Python package and an open Git repository~\cite{gmanePack}.

Data from social networks used in this study were gathered and used within the Anthropological Physics principles~\cite{anPhy}.
