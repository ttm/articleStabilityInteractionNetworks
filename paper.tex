...
\begin{itemize}
    ...
    \item The fraction of participants in each Erd\"os sector is stable along time and can be observed even with very few messages (Section~\ref{subsec:pih}).
    \item The topological measures (regarded as most basic and still representative for a first analysis) combine into principal components in the same way for all lists and all snapshots. 
...
The reminder of this section contains a detailed discussion of these results and Support Information holds further tables and figures from which such results can be better observed.

\subsection{Constancy and discrepancy of activity along time}\label{constDisc}

\begin{table*}[t]
	\caption{The rescaled circular mean $\theta_\mu'$, the standard deviation $S(z)$, the variance $Var(z)$, the circular dispersion $\delta(z)$ and the relation of maximum and minimum incidence at each time unit $\frac{max(incidence}{min(incidence}$. Also, $ \mu_{\frac{max(incidence')}{min(incidence')}} $ and $ \sigma_{\frac{max(incidence')}{min(incidence')} }$ are given for 1000 uniform distribution simulations within the same number of bins and with the same number of samples.
Section~\ref{sec:mtime} exposes the theoretical background considered of directional (or circular) statistics.}
\begin{center}
...
Time scale also revealed patterns through histograms as exposed in Tables~\ref{tab:hin},~\ref{tab:win},~\ref{tab:min} and~\ref{tab:min2}. Seconds, minutes and years scales received the dedicated Sections~\ref{sec:secmin} and~\ref{sec:years} instead of tables.

\subsubsection{Seconds and minutes}\label{sec:secmin}
Messages were slightly more evenly distributed in all lists than in simulations using uniform distribution\footnote{Numpy version 1.6.1, ``random.randint'' function, was used for simulations, algorithms in \url{https://pypi.python.org/pypi/gmane}.}: for both seconds and minutes $\frac{max(incidence)}{min(incidence)} \in (1.26,1.275]$ in the lists. Simulations reach these values but have in average more discrepant higher and lower peaks $\xi=\frac{max(incidence')}{min(incidence')} \Rightarrow \mu_\xi=1.2918 \text{ and } \sigma_\xi=0.04619$.
...
\begin{table}[!h]
	\caption{Variation of activity in the days along the month was not observed, one cannot point much more than a - probably not statistically relevant - tendency of first and second weeks to be more active. The most important trait seems to be homogeneity.}
	\footnotesize
...
\begin{table}[!h]
	\caption{Example of activity percentages of the months along the year. Activity is concentrated in Jun-Aug for MET and LAD, and in Dec-Mar for CPP, LAU and LAD (see Table~\ref{ano}). These observations fit academic calendars, vacations and end-of-year holidays.}
	\footnotesize
	\input{tables/tabMonthsLAD}
\label{tab:min2}
\end{table}

\subsubsection{Activity along years}\label{sec:years}
Literature reports relation of number of participants and hubs stability to community longevity~\cite{barabasiEvo}. Also, activity should follow trends such as economic, climate and technological and periodicity should be observed to some extent. Nevertheless, the time period examined was not sufficient for the analysis of activity along the years (see Section~\ref{sec:conc}). 
...
There is a concentration of hub activity and of vertex with few connections. Table~\ref{autores} exposes this expected distribution of activity among participants in a scale-free context. As we have seen, one way to observe such distribution of participants is through a comparison of the real network with an Erd\"os R\'enyi model.

\begin{table}[!h]
    \caption{Distribution of activity among participants. First column presents the percentage of messages sent by the most active participant. Column for the first quartile ($1Q$) exhibits minimum percentage of participants responsible for at least 25\% of total messages. Similarly, the column for the first three quartiles $1-3Q$ exhibits minimum percentage of participants responsible for 75\% of total messages. The last decile $-10D$ column has maximum percentage of participants responsible for 10\% of messages.}
\begin{center}
...
Accordingly, the distribution of vertices in the  hubs, intermediary, periphery Erd\"os sectors defined in Section~\ref{sectioning} is remarkably stable along time, provided that a sufficiently large sample of 200 or more messages is considered. 
Moreover, the same distribution applies to the networks of all the four email lists, to which are dedicated the various figures in Support Information. 
...
The networks hold their basic structure with as few as 10-50 messages, concentration of activity and the abundance of low-activity participants take place even with very few messages, which is highlighted in the last figures of Support Information.
...
In applying PCA to the snapshots, the contribution of each metric to principal components resulted in very small standard deviation. Table~\ref{tab:pcain} exemplifies the principal components formation with all measures considered, while Support Information gives further assistance to the interested reader.

\begin{table}[!h]
	\caption{Principal components formation with symmetry-related metrics (see Section~\ref{measures}). MET list, $ws=1000$ messages in 20 disjoint positioning was used for statistics. In this case, clusterization is pushed to the third principal component. The second component is primarily derived from symmetry measurements, but also out-degree and out-strength, and disequilibrium standard deviation. Betweenness centrality again has a role similar to degree, but weaker. The clusterization component combines with disequilibrium, while asymmetry is combined to out-degree and out-strength. The three components have in average 80.36\% of the variance.}
	\footnotesize
	\input{tables/tabPCA3CPP}
\label{tab:pcain}
\end{table}

The principal component is an average of centrality measures: degrees, strengths and betweenness centrality.
This composition of the principal component suggests that all degree, strength and betweenness centrality measures are equally important for system characterization, although it is known that they do not relate to the same participation characteristics.
...
This can be observed in Table~\ref{tab:pcain} where the clustering coefficient is only relevant for the third principal component (with contributions from out-degree and out-strength).
...
Such stability of activity along time, Erd\"os sector relative sizes, principal components formation and concentration of variance suggests that understanding types of networks and participants might benefit from a characterization within such framework.
We considered the most trivial case, in which the sector to which a participant belongs is regarded as her/his type. This is useful in deriving more elaborate typologies, and yield a sketch of or a ``primitive'' typology. General considerations:
\begin{itemize}
