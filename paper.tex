...
Messages were downloaded from the GMANE public database~\cite{GMANEwikipedia}. Data annotated from Facebook and Twitter are in a public repository~\ref{fbtwData}. Data from Participa.br was used from the linked data/semantic web RDF triples available in~\cite{rdfParticipa}.
All routines necessary to achieve the results reported in this article, including all tables and figure of the Supporting Information, are available through a public domain Python package and an open Git repository~\cite{gmanePack}.
