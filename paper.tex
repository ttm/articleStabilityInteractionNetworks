...
More recently, the continuous increase of stored data related to human interaction has stimulated research, and overall characterization is scattered in literature.

The present work is aimed at finding common and stable characteristics among (email) interaction networks. This includes observations along time, which imply network evolution, a field that has received attention from the research community
...
Meaningful measures depend heavily on the model and system characteristics~\cite{newmanStru, newmanWeight}, therefore the characterization is restricted to most basic metrics of centrality, clustering and symmetry.
...
Indeed, text mining routines and typologies of online participants benefit from the results here presented~\cite{rcText,rcTipo}. 
 ...
 Next subsection exposes related work. Section~\ref{sec:data} details data while Section~\ref{sec:carac} is dedicated to the characterization methods. Section~\ref{sec:results} holds results and discussions, leading to Section~\ref{sec:conc} for conclusions and further work. Appendix~\ref{ap:ded} develops simple graph results used in the methods, Appendix~\ref{scripts} exposes access to data and scripts, and Appendix~\ref{ap:circular} presents the circular statistics (or directional statistics) used in characterizing time activity. Detailed tables and plots are bundled in Support Information, and are cited upon need.

\subsection{Related work}\label{sec:related}

This work approaches network stability through temporal evolution observation.
The evolution is represented by a timeline of activity snapshots with a constant number of contiguous messages. These snapshots yield a succession of networks. This approach, although coherent and intuitive, seems not to be explored to date~\cite{netWindowEvo}.
...
Controllability of these networks is also an uncovered issue. These has unintuitive properties and might bring into forefront crucial properties~\cite{barabasiControlCapacity,barabasiControlCentrality,barabasiControllability}, which might be the subject of subsequent research.
...
\subsection{Paired article to analyse textual production}\label{subsec:text}
...
This relation holds for each of the lists considered here and a derived article (see Section~\ref{subsec:text}): as the number of participants increases, the number of threads decreases.}
  \label{geralListas}
...
\begin{itemize}
    ...
    \item Mean of asymmetry of edges: $\mu_i^{asy}=\frac{\sum_{j\in J_i} e_{ji}-e_{ij}}{|J_i|=d_i}$. Where $e_{xy}$ is 1 if there is and edge from $x$ to $y$, $0$ otherwise. $|J_i|=d_i$ is the number of neighbors of vertex $i$.
    \item Standard deviation of asymmetry of edges: $\sigma_i^{asy}=\sqrt{\frac{\sum_{j\in J_i}[\mu_{asy} -(e_{ji}-e_{ij}) ]^2  }{d_i}  }$
    \item Disequilibrium: $dis_i=\frac{s_i^{in}-s_i^{out}}{s_i}$.
    \item Mean of disequilibrium of edges: $\mu_i^{dis}=\frac{\sum_{j \in J_i}\frac{w_{ji}-w_{ij}}{s_i}}{d_i}$, where $w_{xy}$ is the weight of edge $x\rightarrow y$ and zero if there is no such edge.
    \item Standard deviation of disequilibrium of edges: $\sigma_i^{dis}=\sqrt{\frac{\sum_{j\in J_i}[\mu_{dis}-\frac{(w_{ji}-w_{ij})}{s_i}]^2}{d_i}}$
\end{itemize}
...
\begin{itemize}
    \item The activity along time is practically the same for all lists (Section~\ref{constDisc}).
    \item The fraction of participants in each connective sector is stable and can be observed even with very few messages (Section~\ref{subsec:pih}).
    \item The measures combine in principal components in the same way and with symmetry related measures presenting more dispersion than clustering coefficient (Section~\ref{prevalence}).
...
\subsection{Scalable fat-tail structure: constancy of membership fractions in the connective sectors}\label{subsec:pih}
